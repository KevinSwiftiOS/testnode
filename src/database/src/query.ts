...
export class Query {
  ...
  where(query: any): Query {
    ...
    if (Object.prototype.toString.call(query).slice(8, -1) !== 'Object') {
      throw new DataBaseError({
        ...
        errMsg: ErrorMsg.QueryParamTypeError,
      });
    }
    ...
    if (keys.length > 0 && !checkFlag) {
      throw new DataBaseError({
        ...ERRORS.INVALID_PARAM,
        errMsg: ErrorMsg.QueryParamValueError,
      });
    }
  ...
  orderBy(fieldPath: string, directionStr: OrderByDirection): Query {
    ...
    if (!OrderDirectionList.includes(directionStr)) {
      throw new DataBaseError({
        ...ERRORS.INVALID_PARAM,
        errMsg: ErrorMsg.DirectionError,
      });
    }
  ...
  limit(limit: number): Query {
    if (!Number.isInteger(limit)) {
      throw new DataBaseError({
        ...ERRORS.INVALID_PARAM,
        errMsg: 'limit' + ErrorMsg.IntergerError,
      });
    }
  ...
  skip(offset: number): Query {
    if (!Number.isInteger(offset)) {
      throw new DataBaseError({
        ...ERRORS.INVALID_PARAM,
        errMsg: 'skip' + ErrorMsg.IntergerError,
      });
    }
  ...
  async remove(): Promise<RemoveRes> {
    ...
    if (
      ...
      throw new DataBaseError({
        ...ERRORS.INVALID_PARAM,
        errMsg: 'remove()操作不支持skip,limit,projection,orderBy',
      });
    }
    // multi 再确认下
